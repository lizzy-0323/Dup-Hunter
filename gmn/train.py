...
f_train=open("result/train.txt","w",encoding="UTF-8")
f_valid=open("result/valid.txt","w",encoding="UTF-8")
f_test=open("result/test.txt","w",encoding="UTF-8")
"""Evaluation"""
...
def load_data():
    ...
    for i in range(len(names)):
        with open("../data/remove_9.{}".format(names[i]), 'rb') as f:
...
if os.path.exists('Model/checkpoint'):
    print("有")
    saver.restore(sess, 'Model/lyl.GMN-107')
else:
    print("无")
...
for i_iter in range(config["training"]["n_training_steps"]):
    ...
    if i_iter % 5 == 0:
        saver.save(sess, "Model/%d/lyl.GMN" % (i_iter), global_step = 9)
    for i in range(0, len(train_graphs), batch_size):
        ...
        # for s in sim:
        #     s[0]
        # print("label", train_metrics["labels"])
